import { logout, getCurrentUser, saveUser, updateCurrentPassword, addEmail, updatePrimaryEmail, deleteUserEmail, getProfileById, saveActivityTypes } from '../models/user.model'
...
/**
 * Remove given passport country from the user's profile
 * @param countryName passport country to remove from user's profile
 * @param profileId id of user whose profile should be updated
 */
 ...
 */
export async function addActivityType(activityType: string, profileId: number) {
    let user = await getProfileById(profileId);
    if (user === null) {
        throw new Error("No active user found.");
    }
 ...
 */
export async function removeActivityType(activityType: string, profileId: number) {
    let user = await getProfileById(profileId);
    if (user === null) {
        throw new Error("No active user found.");
    }
