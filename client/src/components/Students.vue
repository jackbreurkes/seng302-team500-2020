      ...
      <div v-else>
        ...
        <section class="main" v-show="students.length" v-cloak>
          ...
          <ul class="student-list">
            <li v-for="student in filteredStudents"
                class="student"
                :key="student.id"
                :class="{ checked: student.checked, editing: student == editedStudent }">
              <div class="view">
                <input class="toggle" type="checkbox" v-model="student.checked" >
                <label @dblclick="editStudent(student)">{{ student.name }}</label>
                <button class="destroy" @click="removeStudent(student)"></button>
              </div>
        ...
        <footer class="footer" v-show="students.length" v-cloak>
          ...
          <ul class="filters">
            ...
            <li><a href="#/checked" @click="setVisibility('checked')" :class="{ selected: visibility == 'checked' }">Checked</a></li>
          </ul>
          <button class="clear-checked" @click="removeChecked" v-show="students.length > remaining">
            Delete checked
          </button>
        </footer>
      </div>
...
<script>
  ...
  const Students = {
    ...
    removeStudent: function (student) {
      api.removeForId(student.id).then(() => {
    ...
    doneEdit: function (student) {  
      ...
      api.updateForId(student.id, student.name.trim()).then((response) => {
